...
Локально гоняются на изолированной SQLite in-memory базе (см.
`tests/conftest.py`) — быстро и без внешних зависимостей, боевая PostgreSQL
не трогается. В CI переменная `TEST_DATABASE_URL` переключает их на
...
**Текущее покрытие `backend/app/` — 97% (58 тестов).** Непокрытыми остались
дефолтные значения дат в `/stats/*` и одна защитная ветка, недостижимая при
нормальной работе. `bot/bot.py` и `client/tracker.py` (почти 900 строк
суммарно) — пока 0%: их невозможно импортировать в CI, потому что
`ctypes.windll` и `aiogram.Bot` выполняются на уровне модуля. Это
разблокируется рефакторингом чистой логики в отдельные модули.
...
Осталось: рефакторинг client/bot на тестируемую чистую логику, тесты
клиента и бота, порог покрытия в CI.
...
- [x] CI (GitHub Actions) — `.github/workflows/tests.yml`, автозапуск
      `pytest tests/` при пуше/PR в main на настоящем Postgres
      (service-контейнер), а не на SQLite — иначе часть тестов
      ложно-зелёные; подтверждено живым зелёным прогоном на GitHub
- [ ] Полировка тестового покрытия до полного доверия — план из 16
      пунктов, заведён отдельными GitHub Issues. Backend закрыт
      (89% с оговорками → 97%, 32 теста → 58); осталось 8 пунктов:
      рефакторинг `bot/bot.py`/`client/tracker.py` на тестируемую чистую
      логику, тесты клиента и бота, порог покрытия в CI
