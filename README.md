...
- [ ] Привязка старой истории (7446 записей на текущем `id=3`) к
      реальному `telegram_id` — чтобы не расщеплять свою историю на два
      разных аккаунта после перехода на Telegram-логин в боте
